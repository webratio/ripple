...
function _buildDOMNode(setting, settingType, key) {
    ...
    case "range":
        ...
        if (typeof setting.suggestedOptionsProvider === "function") {
            jNode.bind("click", function () {
                var suggestedValues = setting.suggestedOptionsProvider();
                
                var jSeparatorOption = jNode.data("separatorOption");
                if (suggestedValues.length <= 0) {
                    if (jSeparatorOption) {
                        jSeparatorOption.remove();
                        jNode.removeData("separatorOption");
                    }
                } else {
                    if (!jSeparatorOption) {
                        jSeparatorOption = jQuery("<option/>").attr("disabled", "").text("--------------------");
                        jSeparatorOption.appendTo(jNode);
                        jNode.data("separatorOption", jSeparatorOption);
                    }
                }
                
                function getOptionScore(option) {
                    if (option === jSeparatorOption && jSeparatorOption[0]) {
                        return 1;
                    }
                    if (suggestedValues.indexOf(option.value) >= 0) {
                        return 0;
                    }
                    return 2;
                }
                
                var jOptions = jNode.find("option");
                jOptions.detach();
                jOptions.sort(function(o1, o2) {
                    if (o1 === o2) {
                        return 0;
                    }
                    var score1 = getOptionScore(o1);
                    var score2 = getOptionScore(o2);
                    if (score1 !== score2) {
                        return (score1 < score2 ? -1 : 1);
                    }
                    var text1 = jQuery(o1).text();
                    var text2 = jQuery(o2).text();
                    return text1.localeCompare(text2);
                });
                jOptions.appendTo(jNode);
            });
        }
    }
    ...
    initialize: function () {
        ...
        utils.forEach(settings, function (settingSection, settingType) {
            ...
            utils.forEach(settingSection, function (setting, key) {
                ...
                if (setting.callback) {
                    setting.callback(deviceSettings.retrieve(settingType + "." + key));
                }
            });
        });
    }
