...
module.exports = {
    initialize: function () {
        $("#exec-dialog").dialog({
            autoOpen: false,
            modal: true,
            title: "Missing plugin emulation support",
            width: 500,
            position: 'center'
        }).hide();
    ...
    show: function (service, action, success, fail) {
        ...
        // Disable buttons if there is no corresponding callback
        $("#exec-success").prop('disabled', !success);
        $("#exec-fail").prop('disabled', !fail);
