...
module.exports = {
    ...
    initialize: function () {
        ...
        jQuery("#settings-resetlocaldata").click(function () {
            var promises = ripple('emulatorBridge').window().wr.mvc.mgmt.DatabaseConfig.getAll().map(function(db){
                return db.clear();
            });         
            Promise.all(promises).then(function() {
                console.log("Clearing local storage info");
                ripple('emulatorBridge').window().wr.mvc.mgmt.Runtime.get().clearLocalData();
        ...
        jQuery("#platform-events-fire-suspend").click(function () {
            ...
            args = spec.events[eventName].args ? document.getElementById("platform-events-args").value : null,
                    callback = spec.events[eventName].callback;
            console.log('Pressed "Suspend" button. Application is now running in background.');
