...
function _collectPreferences(json) {
    var result = {};
    
    var root = json["widget"][0];
    var platformRoots = {};
    root["platform"].forEach(function(platform) {
        var platformName = platform["@attributes"]["name"];
        platformRoots[platformName] = platform;
        result[platformName] = {};
    });
    
    (root["preference"] || []).forEach(function(preference) {
        Object.keys(platformRoots).forEach(function(platformName) {
            result[platformName][preference["@attributes"]["name"]] = preference["@attributes"]["value"];
        });
    });
    Object.keys(platformRoots).forEach(function(platformName) {
        (platformRoots[platformName]["preference"] || []).forEach(function(preference) {
            result[platformName][preference["@attributes"]["name"]] = preference["@attributes"]["value"];
        });
    });
...
module.exports = {
    ...
    initialize: function () {
        ...
        try {
            ...
            if (xmlHttp.responseXML) {
                ...
                _process();
                _configValidationResults = null;
                ripple('ui/plugins/widgetConfig').initialize();
            }
        }
        catch (e) {
            exception.handle(e);
    ...
    getPreference: function (name, defaultValue) {
        var platform = ripple('devices').getCurrentDevice().osName;
        if (!_configPreferences) {
            _configPreferences = _collectPreferences(_configValidationResults.rawJSON);
        }
        var platformPrefx = _configPreferences[platform];
        var value = (platformPrefx && platformPrefx[name]) || _configPreferences[name];
