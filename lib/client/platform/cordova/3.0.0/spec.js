...
module.exports = {
    ...
    initialize: function (win) {
        var honeypot = ripple('honeypot'),
            ...
            set = function (orig) {
                ...
                cordova.define("cordova/exec", function (require, exports, module) {
                    ...
                    module.exports.init = function(){cordova.require('cordova/channel').onNativeReady.fire();}; // RIPPLE-69 adding cordova-3.6.x support; RIPPLE-87 init method overwritting so that its closure points to the current cordova object
                });
            };
        ...
        // Don't display I Haz Cheeseburger?!?! dialog for ionic keyboard plugin init call
        bridge.add("Keyboard", ripple('platform/cordova/3.0.0/bridge/keyboard'));

        // Don't display I Haz Cheeseburger?!?! dialog for statusbar plugin _ready call
        bridge.add("StatusBar", ripple('platform/cordova/3.0.0/bridge/statusbar'));

        honeypot.monitor(win, "cordova").andRun(get, set);

        var topCordova = ripple('platform/cordova/2.0.0/spec');
