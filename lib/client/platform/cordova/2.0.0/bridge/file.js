    ...
    win.document.addEventListener("deviceready", function() {
        Object.defineProperty(win.File.prototype, "localURL", {
            ...
            set: function(new_value){
                this._localURL = new_value.replace(/cdvfile:\/\/localhost\//i, "filesystem:" + document.location.origin + "/");
            }
        });
        win.Entry.prototype.toInternalURL = (function() {
            ...
            return function() {
                ...
                if (result) {
                    result = result.replace(document.location.origin.replace("://","_").replace(":", "_") + ":Temporary", "temporary");    
                }
                return result;
            };
        })();
        win.Entry.prototype.toURL = (function() {
            var $original = win.Entry.prototype.toURL;
            return function() {
                var result = $original.apply(this, arguments);
                if (result) {
                    var prefix = document.location.origin + "/__cdvfile_temporary__/";
                    if (result.indexOf(prefix) === 0) {
                        result = result.replace(prefix, "filesystem:" + document.location.origin + "/temporary/");
                    }
                }
                return result;
            };
        ...
        (function() {
            var calling = false; // to avoid recursion if 'toURL' uses 'nativeURL'
            Object.defineProperty(win.Entry.prototype, "nativeURL", {
                get: function() {
					var proto = Object.getPrototypeOf(this);
                    if (proto.nativeURL || calling) {
                        return proto.nativeURL;
                    }
                    calling = true;
                    try {
                        return this.toURL();
...
function cleanPath(path) {
    if (path.indexOf(document.location.origin + "/__cdvfile_temporary__/" === 0)) {
        path = path.replace(document.location.origin + "/__cdvfile_temporary__/", "cdvfile://localhost/temporary/");
    } else if (path.indexOf("filesystem:" + document.location.origin + "/") === 0) {
        path = path.replace("filesystem:" + document.location.origin + "/", "cdvfile://localhost/");
    }
