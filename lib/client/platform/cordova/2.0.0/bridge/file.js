...
(function() {
    ...
    win.document.addEventListener("deviceready", function() {
        ...
        win.FileEntry.prototype._normalizeURL = (function () {
            return function () {
                var localURL = arguments[0];
                //var nativeURL = arguments[1];
                var result = localURL.replace(/cdvfile:\/\/localhost\//i, "filesystem:" + document.location.origin + "/");
                return result;
            };
        })();
    });
...
function cleanPath(path) {
    if (path.indexOf("filesystem:" + document.location.origin + "/") !== -1) {
