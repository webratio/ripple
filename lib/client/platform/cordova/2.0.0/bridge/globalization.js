...
var platform = ripple('platform'),
    ...
    patternConverter = ripple('platform/cordova/2.0.0/bridge/patternConverter'),
    numberSeparators = ripple('platform/cordova/2.0.0/bridge/numberSeparators'),
...
function parse(str, length, selector) {
    if (selector === "time") {
        return moment(str, moment.localeData().longDateFormat('LT'));
    }
    ...
    switch (length) {
    case 'medium':
        format = moment.localeData().longDateFormat('LL') + " LT";
        break;
    case 'long':
        format = moment.localeData().longDateFormat('LLL');
        break;
    case 'full':
        format = moment.localeData().longDateFormat('LLLL');
        break;
    case 'short':
    ...
    format = format.replace("LT", moment.localeData().longDateFormat('LT'));
...
module.exports = {
    getLocaleName: function (win) {
        win({ value: moment.locale() });
    },

    getPreferredLanguage: function (win) {
        win({
            value: platform.current().device.globalization.locale.options[moment.locale()]
        });
    },
    ...
    getDatePattern: function (win, fail, args) {
        ...
        if (selector === 'time') {
            pattern = moment.localeData().longDateFormat('LT');
        }
        else {
            switch (formatLength) {
            case 'medium':
                pattern = moment.localeData().longDateFormat('LL') + " LT";
                break;
            case 'long':
                pattern = moment.localeData().longDateFormat('LLL');
                break;
            case 'full':
                pattern = moment.localeData().longDateFormat('LLLL');
                break;
            case 'short':
                pattern = moment.localeData().longDateFormat('L') + " LT";
                break;
            }
            ...
            pattern = pattern.replace("LT", moment.localeData().longDateFormat('LT'));
        }

        pattern = patternConverter.convert(pattern);
    ...
    getDateNames: function (win, fail, args) {
        ...
        if (item === 'months'){
            for (var m = 0; m < 12; m++){
                names[m] = moment[key]('', m);
            }
        }
        if (item === 'days'){
            for (var d = 0; d < 7; d++){
                names[d] = moment[key](false, '', d);
            }
        }
        ...
        win({value: new bridge.window().Array().concat(names)});
    },
    ...
    getNumberPattern: function (win, fail, args) {
        var localeForPattern = moment.locale().toLowerCase();
        var pattern = "#,##0.###",
        ...
        win({
            ...
            positive: "",
            negative: "-",
            decimal: numberSeparators[localeForPattern]["decimal"],
            grouping: numberSeparators[localeForPattern]["group"]
        });
    },
