...
function _error(e, baton) {
    ...
    if (e.name === "SecurityError" || e["code"] === 2 /* FileError.SECURITY_ERR */) {
        msg = 'SecurityError: Looks like you\'re running in the file:/// scheme. You need to start Chrome with --allow-file-access-from-files if you want to have access to the FileSystem API.';
    } else if (e.name) {
        msg = e.name;
    } else if (e["code"]){
        switch (e["code"]) {
...
_self = {
    ...
    write: function (path, contents, success, error, options) {
        ...
        _self.stat(path, function (entry) {
            ...
            if (e.name === "NotFoundError" || e["code"] === 1 /* FileError.NOT_FOUND_ERR */) {
                _self.touch(path, write, error, options);
            } else {
                error(e);
            }
